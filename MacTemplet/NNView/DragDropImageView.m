...
//            self.superview.frame = self.frame;
        ...
        self.window.title = fileURL != NULL ? self.path.lastPathComponent : @"(no name)";
        if ([self.delegate respondsToSelector:@selector(dropComplete:name:)]) {
            [self.delegate dropComplete:self.path name:self.window.title];
        }
    }
    return YES;
}
...
- (NSArray *)namesOfPromisedFilesDroppedAtDestination:(NSURL *)dropDestination{
    ...
    BOOL result = [bitmapData writeToFile:[dropDestination.path stringByAppendingPathComponent:filename] atomically:YES];
    if ([self.delegate respondsToSelector:@selector(dragComplete:name:success:)]) {
        [self.delegate dragComplete:self.path name:filename success:result];
    }
...
//    [self.image dissolveToPoint: NSZeroPoint fraction: 1.0];
    [self.image drawAtPoint:NSZeroPoint fromRect:NSZeroRect operation:NSCompositingOperationSourceOver fraction:1.0];
    ...
    dragImage.size = scaledImageSize;//change to the size we are displaying
